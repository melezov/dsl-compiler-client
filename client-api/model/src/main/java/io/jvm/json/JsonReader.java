...
public class JsonReader {
    ...
    /** Always read the next character, but doesn't throw an IOException on stream EOF.
    /*  For usage with number parser, when there is no specific token to end the parser
     */
    ...
    /** Returns the last read character if it hasn't been invalidated, reads the next one otherwise */
    public char read() throws IOException {
        return _lastValid ? _last : next();
    }

    /** Invalidates the last read character */
    public void invalidate() throws IOException {
        _lastValid = false;
    }

    /** Reads the next character and invalidates the last character afterwards (so that the next read() will always read) */
    ...
    /** Reads the last character and invalidates it (so that the next read() will always read) */
    ...
    /** Reads the last character or invalidates the one from cache ((so that the next read() will always read) */
    public void assertRead(final char expected) throws IOException {
        if (_lastValid) assertLast(expected); else assertNext(expected);
    }

    /** Always reads the next character and tries to parse it as a hexadecimal digit */
    ...
    /** Assert that the current token is 'null' (first character already read() */
    ...
    /** Assert that the current token is 'true' (first character already read() */
    ...
    /** Assert that the current token is 'false' (first character already read() */
    ...
    /** Parses the next numeric expression and returns it in string format, does not evaluate it */
    ...
    /** Array can be null, elements can be null */
    @SuppressWarnings("unchecked")
    ...
    /** List can be null, elements can be null */
    @SuppressWarnings("unchecked")
    ...
    /** Set can be null, elements can be null */
    @SuppressWarnings("unchecked")
